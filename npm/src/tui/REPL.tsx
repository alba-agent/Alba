...
function MessageLine({ msg }: { msg: ChatMessage }) {
  ...
  if (msg.role === "user") {
    return (
      <Box flexDirection="row" gap={1} marginTop={1}>
        <Text color={ALBA_COLORS.muted}>{time}</Text>
        <Text color={ALBA_COLORS.accent} bold>you  </Text>
  ...
  if (msg.role === "assistant") {
    return (
      <Box flexDirection="row" gap={1} marginTop={1}>
        <Text color={ALBA_COLORS.muted}>{time}</Text>
        <Text color={ALBA_COLORS.header} bold>🪼   </Text>
        <Text wrap="wrap">{msg.content}</Text>
      </Box>
    );
  }
    ...
    return (
      <Box flexDirection="column" marginY={0}>
        <Box flexDirection="row" gap={1}>
          ...
          {isLong && <Text color={ALBA_COLORS.dim}> ({msg.content.length} chars)</Text>}
        </Box>
        {!isLong && (
          <Box marginLeft={6}>
            <Text color={ALBA_COLORS.muted} wrap="wrap">{msg.content}</Text>
          </Box>
        )}
      </Box>
    );
  ...
  if (msg.role === "notify") {
    return (
      <Box
        borderStyle="round"
        borderColor={ALBA_COLORS.accent}
        marginY={1}
        paddingX={1}
      >
        <Text wrap="wrap">{msg.content}</Text>
  ...
  return (
    <Box flexDirection="row" gap={1}>
      <Text color={ALBA_COLORS.dim} wrap="wrap">{msg.content}</Text>
    </Box>
  );
}
  ...
  return (
    <Box flexDirection="column" width={termWidth}>
      ...
      <Box flexDirection="column" flexGrow={1}>
        ...
        {streamingText && (
          <Box flexDirection="row" gap={1} marginTop={1}>
            <Text color={ALBA_COLORS.muted}>   </Text>
            <Text color={ALBA_COLORS.header} bold>🪼   </Text>
        ...
        {toolRunning && (
          <Box flexDirection="row" gap={1} marginTop={1}>
            <Text color={ALBA_COLORS.warn}>⚙  running {toolRunning}…</Text>
          </Box>
        )}
      </Box>
      ...
      <Box
        borderStyle="round"
        borderColor={disabled ? ALBA_COLORS.dim : ALBA_COLORS.accent}
        paddingX={1}
        marginTop={1}
      >
        <Text color={ALBA_COLORS.accent}>› </Text>
        <TextInput
          value={input}
          onChange={setInput}
