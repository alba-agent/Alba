...
const CACHE_FILE  = join(ALBA_HOME, "models.json");
...
export class ModelRegistry {
  ...
  async initialise(apiKey?: string): Promise<void> {
    ...
    try {
      const res = await fetch("https://openrouter.ai/api/v1/models", {
        headers: { Authorization: `Bearer ${key}`, "User-Agent": "ALBA/1.0" },
  ...
  buildConfig(modelId: string, maxTokens: number, temperature: number, tier?: ModelTier): ModelConfig | null {
    ...
    if (env.OPENROUTER_API_KEY) {
      return {
        ...
        maxTokens,
        temperature,
        siteUrl:     env.OPENROUTER_SITE_URL  ?? "https://alba.fun",
        siteName:    env.OPENROUTER_SITE_NAME ?? "ALBA",
      };
    }
  ...
  /** Get the full TieredModel entry for a model ID (used by CostTracker). */
  getModelEntry(modelId: string): TieredModel | undefined {
