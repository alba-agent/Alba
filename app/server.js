...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: "binance_prices",
      description: "Get current prices for Binance trading pairs",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get order book depth from Binance",
      parameters: {
        type: "object",
...
async function executeTool(name, args) {
  try {
    switch (name) {
      ...
      case "binance_prices": {
        return await getBinancePrices(args.symbols);
      }
      case "binance_klines": {
        return await getBinanceKlines(args.symbol, args.interval, args.limit);
      }
      case "binance_order_book": {
        return await getBinanceOrderBook(args.symbol, args.limit);
      }

      case "binance_order_book": {
        return await getBinanceOrderBook(args.symbol, args.limit);
      }
      case "hyperliquid_info": {
        return await getHyperliquidInfo(args.user);
      }
      case "hyperliquid_funding": {
        return await getHyperliquidFunding();
      }
      case "hyperliquid_leaderboard": {
        return await getHyperliquidLeaderboard();
      }
      case "jupiter_quote": {
        const url = new URLSearchParams({ inputMint: args.inputMint, outputMint: args.outputMint, amount: args.amount });
      ...
      case "raydium_swap": {
        const url = new URLSearchParams({ inputMint: args.inputMint, outputMint: args.outputMint, amount: args.amount, txVersion: 'V0' });
...
// ── Binance tool helpers ───────────────────────────────────────────────────────────
async function getBinancePrices(symbols = ["BTCUSDT", "ETHUSDT", "SOLUSDT"]) {
...
async function getBinanceKlines(symbol, interval = "1h", limit = 100) {
  ...
  return JSON.stringify(klines, null, 2);
}

async function getBinanceOrderBook(symbol, limit = 100) {
...
// ── Hyperliquid tool helpers ───────────────────────────────────────────────────
async function getHyperliquidInfo(user) {
  if (!user) return "HYPERLIQUID_WALLET_ADDRESS required";
...
async function getHyperliquidLeaderboard() {
  const resp = await fetch('https://api.hyperliquid.xyz/info', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
